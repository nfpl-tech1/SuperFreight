...
import { Send, Bold, Italic, Underline, List as ListIcon, ListOrdered, RefreshCcw, FileText } from "lucide-react";
...
export function RFQPreview({
    ...
    const stuffingType = typeof values.stuffing_type === "string"
        ? stuffingTypeMap[values.stuffing_type] ?? undefined
        : undefined;

    // Trade lane from form values (if present) or from inquiry prop
    const effectiveTradeLane = typeof values.trade_lane === "string"
    ...
    const matchedTemplate = useMemo(
        ...
        [departmentId, values, effectiveTradeLane, incoterm, stuffingType, isExwClubbed],
    );
    ...
    useEffect(() => {
        ...
        const tableDOM = editorRef.current.querySelector("#rfq-dynamic-table-container table");

        // Use template-based rendering when a template matches
        if (matchedTemplate) {
            const resolveValue = isEdited && tableDOM
                ? (row: { label: string; value: string }) => {
                    const prev = prevRowsRef.current.find((prevRow) => prevRow.label === row.label);
                    if (prev && prev.value === row.value) {
                        const cell = Array.from(tableDOM.querySelectorAll("td[data-value-label]")).find(
            ...
            if (!isEdited) {
                editorRef.current.innerHTML = buildTemplatePreviewEmailHtml(matchedTemplate, values, resolveValue);
            } else {
                const tableContainer = editorRef.current.querySelector("#rfq-dynamic-table-container");
                if (tableContainer) {
                    // Re-render only the data table, keep user edits to text
                    const tempDiv = document.createElement("div");
                    tempDiv.innerHTML = buildTemplatePreviewEmailHtml(matchedTemplate, values, resolveValue);
                    const newTable = tempDiv.querySelector("#rfq-dynamic-table-container");
            ...
            if (!isEdited) {
                editorRef.current.innerHTML = buildPreviewEmailHtml({ departmentId, tableHtml });
            } else {
                const tableContainer = editorRef.current.querySelector("#rfq-dynamic-table-container");
    ...
    const subjectLine = buildPreviewSubjectLine(
        departmentId,
        ...
        <div className="flex h-full flex-col overflow-hidden bg-white">
            <div className="flex flex-col gap-1.5 border-b border-border px-3 py-2.5 sm:flex-row sm:items-baseline">
                ...
                <span className="text-[0.6875rem] text-muted-foreground">
                    {isEdited ? "Auto-update paused to protect your edits" : "Auto-updates as you fill the form"}
                </span>
            </div>

            <div className="flex flex-wrap items-center gap-1 border-y border-slate-200 bg-slate-50 px-3 py-1.5">
                <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-600" onClick={(event) => handleFormat(event, "bold")}>
                    <Bold className="h-[15px] w-[15px]" />
                </Button>
                <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-600" onClick={(event) => handleFormat(event, "italic")}>
                    <Italic className="h-[15px] w-[15px]" />
                </Button>
                <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-600" onClick={(event) => handleFormat(event, "underline")}>
                    <Underline className="h-[15px] w-[15px]" />
                </Button>
                <div className="mx-1 h-4 w-px bg-slate-300" />
                <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-600" onClick={(event) => handleFormat(event, "insertOrderedList")}>
                    <ListOrdered className="h-[15px] w-[15px]" />
                </Button>
                <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-600" onClick={(event) => handleFormat(event, "insertUnorderedList")}>
            ...
            <div className="flex-1 overflow-y-auto px-3 pb-3 pt-3 text-[0.8125rem] leading-relaxed text-[hsl(215_20%_25%)]">
                <p className="mb-3 break-words border-b border-slate-100 pb-3 text-[0.8125rem] text-[hsl(228_55%_40%)]">
