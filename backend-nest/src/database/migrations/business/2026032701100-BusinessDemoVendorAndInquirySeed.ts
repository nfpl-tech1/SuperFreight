...
import { BusinessVendorMasterPhase12026032400400 } from './2026032400400-BusinessVendorMasterPhase1';
import { BusinessVendorLocationSourcing2026032700800 } from './2026032700800-BusinessVendorLocationSourcing';

type ServiceLocationSeed = {
  id: string;
  name: string;
...
type OfficeSeed = {
  ...
  typeCodes: string[];
  portRefs: Array<{ name: string; country: string; isPrimary?: boolean }>;
  serviceLocationIds: string[];
  contacts: ContactSeed[];
  ccRecipients: Array<{ id: string; email: string }>;
};
...
type InquirySeed = {
  ...
  status:
    ...
    | 'CLOSED';
  customerName: string;
  customerRole: 'Consignee' | 'Shipper';
  tradeLane: 'Export' | 'Import' | 'Cross Trade';
  origin: string;
  destination: string;
  shipmentMode: 'FCL' | 'LCL' | 'AIR';
  incoterm: string;
  cargoSummary: string;
  createdAt: string;
};
...
type JobServicePartSeed = {
  id: string;
  jobId: string;
  partType: 'FREIGHT' | 'CHA' | 'TRANSPORTATION';
  status: string;
  applicationSlug: string;
  meta: Record<string, string>;
};

const SERVICE_LOCATIONS: ServiceLocationSeed[] = [
  {
...
const INDIA_VENDOR_SEEDS: VendorSeed[] = [
  {
    ...
    offices: [
      {
        ...
        specializationRaw:
          'Factory pickup, port drayage, and linehaul coordination.',
...
const OVERSEAS_VENDOR_SEEDS: VendorSeed[] = [
  {
    ...
    offices: [
      {
        ...
        specializationRaw:
          'Destination handling, deconsolidation, and customs.',
        doesSeaFreight: true,
        doesOwnCustomClearance: true,
        doesOwnWarehousing: true,
        typeCodes: ['WCA_AGENT', 'CHA'],
...
const VENDORS: VendorSeed[] = [...INDIA_VENDOR_SEEDS, ...OVERSEAS_VENDOR_SEEDS];
...
const JOBS: JobSeed[] = INQUIRIES.map((inquiry, index) => ({
...
const JOB_SERVICE_PARTS: JobServicePartSeed[] = [
  {
...
async function ensureVendorSchema(queryRunner: QueryRunner) {
  if (!(await queryRunner.hasTable('vendor_master'))) {
    await new BusinessVendorMasterPhase12026032400400().up(queryRunner);
  }

  if (!(await queryRunner.hasTable('service_location_master'))) {
    await new BusinessVendorLocationSourcing2026032700800().up(queryRunner);
  }
}
...
async function findVendorTypeId(queryRunner: QueryRunner, typeCode: string) {
...
export class BusinessDemoVendorAndInquirySeed2026032701100 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const allTypeCodes = Array.from(
      new Set(
        VENDORS.flatMap((vendor) =>
          vendor.offices.flatMap((office) => office.typeCodes),
        ),
      ),
    );
    ...
    for (const vendor of VENDORS) {
      ...
      for (const office of vendor.offices) {
        ...
        for (const [
          index,
          serviceLocationId,
        ] of office.serviceLocationIds.entries()) {
    ...
    for (const [index, job] of JOBS.entries()) {
      const createdAt = new Date(
        INQUIRIES[index]?.createdAt ?? new Date().toISOString(),
      );
