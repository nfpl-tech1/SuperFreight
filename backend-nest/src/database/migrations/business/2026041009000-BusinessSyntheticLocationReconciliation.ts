...
type SyntheticServiceLocationMapping = {
...
type SyntheticSeaportSplitMapping = {
  syntheticCode: string;
  targets: SplitTarget[];
  notes: string;
};
...
const SYNTHETIC_SERVICE_LOCATION_MAPPINGS: SyntheticServiceLocationMapping[] = [
  {
...
const SYNTHETIC_SEAPORT_SPLIT_MAPPINGS: SyntheticSeaportSplitMapping[] = [
  {
...
async function ensureServiceLocation(
  queryRunner: QueryRunner,
  mapping: SyntheticServiceLocationMapping,
) {
...
async function ensureCanonicalPort(
  queryRunner: QueryRunner,
  target: SplitTarget,
  notes: string,
) {
...
async function migrateSyntheticPortToServiceLocation(
  ...
  const officeLinks = (await queryRunner.query(
    ...
    `,
    [syntheticPort.id],
  )) as Array<{ id: string; officeId: string }>;
  ...
  await queryRunner.query(
    `DELETE FROM "vendor_office_ports" WHERE "portId" = $1`,
    [syntheticPort.id],
  );
  await queryRunner.query(
    `DELETE FROM "port_alias" WHERE "portId" = $1`,
    [syntheticPort.id],
  );
  await queryRunner.query(
    `
...
async function splitSyntheticPort(
  queryRunner: QueryRunner,
  mapping: SyntheticSeaportSplitMapping,
) {
  const syntheticPort = await findPortByCode(
    queryRunner,
    mapping.syntheticCode,
    'SEAPORT',
  );
  if (!syntheticPort) {
    return;
...
export class BusinessSyntheticLocationReconciliation2026041009000
  implements MigrationInterface
{
  ...
  public async down(): Promise<void> {
    // This migration intentionally performs no automatic rollback because it
    // migrates live business links from synthetic locations to canonical ones.
  }
}
